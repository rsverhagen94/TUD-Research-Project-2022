        ...
        while True:
            ...
            if Phase.PICK_UNSEARCHED_ROOM == self._phase:
                ...
                if self._remaining_zones and len(unsearched_rooms) == 0:
                    if self._tactic == 'defensive':
                        ...
                        self._send_message('Going to re-explore all offices to extinguish fires.', 'Brutus')
                    if self._tactic == 'offensive':
                        self._searched_rooms_offensive = []
                        self._lost_victims = []
                        self._searched_rooms_offensive.append(self._door['room_name'])
                        self._offensive_search_rounds += 1
                        self._send_message('Going to re-explore all offices to rescue victims.', 'Brutus')
                    self._send_messages = []
                    self._fire_locations = []
            ...
            if Phase.FOLLOW_PATH_TO_ROOM == self._phase:
                ...
                if self._tactic == 'offensive' and not self._goal_victim:
                    self._send_message('Moving to ' + str(self._door['room_name']) + ' to search for victims because it is the closest not explored office.', 'Brutus')     
                if self._tactic == 'offensive' and self._goal_victim:
                    self._send_message('Moving to ' + str(self._door['room_name']) + ' to see if ' + self._goal_victim + ' can be rescued now.', 'Brutus')  
                if self._tactic == 'defensive':
                    self._send_message('Moving to ' + str(self._door['room_name']) + ' to search for fire because it is the closest not explored office.', 'Brutus')           
